"""End-to-end test on a tiny randomly initialized LLaVA-Next (no downloads).

Anyres: token count per image depends on its resolution (base tile +
high-res patches + newline separators), so spans are variable.
"""
...
from transformers import LlavaNextConfig, LlavaNextForConditionalGeneration  # noqa: E402
...
def model():
    ...
    torch.manual_seed(0)
    m = LlavaNextForConditionalGeneration(cfg)
    m.eval()
    return m
...
def test_anyres_variable_spans(model):
    # two images at different resolutions -> different token counts
